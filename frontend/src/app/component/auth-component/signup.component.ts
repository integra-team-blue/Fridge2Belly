      ...
      <section class="auth-card">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          <div class="field">
            ...
            <small *ngIf="email.touched && email.hasError('email')">Please enter a valid email.</small>
          ...
          <div class="field">
            ...
            <small *ngIf="username.touched && username.hasError('minlength')">Minimum 3 characters.</small>
            <small *ngIf="username.touched && username.hasError('taken')">Username already taken.</small>
          </div>

          <div class="field">
            <label>Password</label>
            <p-password formControlName="password" [feedback]="true" [toggleMask]="true"></p-password>
            <small *ngIf="password.touched && password.errors?.['weak']">At least 8 chars, upper & lower case, and a number.</small>
          ...
          <div class="field">
            ...
            <small *ngIf="confirmPassword.touched && confirmPassword.errors?.['mismatch']">Passwords do not match.</small>
            <small *ngIf="confirmPassword.touched && confirmPassword.hasError('required')">Confirm password is required.</small>
          </div>

          <button pButton type="submit" [disabled]="loading.active() || form.pending || form.invalid" [label]="loading.active() ? 'Creating account…' : 'Create account'"></button>
        </form>
...
export class SignupComponent {
  ...
  form = this.fb.nonNullable.group({
    ...
    password: ['', [Validators.required, (c: AbstractControl) => strong(c.value ?? '') ? null : { weak: true }]],
  ...
  submit() {
    ...
    if (v.password !== v.confirmPassword) this.confirmPassword.setErrors({ mismatch: true });
    if (this.form.pending || this.form.invalid) { this.form.markAllAsTouched(); return; }
    const payload = { username: v.username, email: v.email };
    this.loading.show();
    this.auth.signup(payload).subscribe({
      next: () => { this.toast.push('Account created. Welcome!', 'success'); this.router.navigateByUrl('/dishes'); },
      error: (e: HttpErrorResponse) => this.toast.push(e.error?.message || e.message || 'Signup failed', 'error'),
