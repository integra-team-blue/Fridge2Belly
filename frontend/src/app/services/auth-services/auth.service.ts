...
export interface UserDto { id?: string; username: string; email: string; }
...
export class AuthService {
  ...
  isUsernameTaken(username: string): Observable<boolean> {
    return this.getUsers().pipe(map(list => list.some(u => u.username.toLowerCase() === username.toLowerCase())));
  }

  isEmailTaken(email: string): Observable<boolean> {
    return this.getUsers().pipe(map(list => list.some(u => u.email.toLowerCase() === email.toLowerCase())));
  }
